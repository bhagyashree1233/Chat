angular.module('elastichat', ['ionic', 'monospaced.elastic', 'angularMoment','ngCordova'])
.run(function($ionicPlatform, $cordovaSQLite, $rootScope) {
  $ionicPlatform.ready(function() { 
    ...
    $cordovaSQLite.execute($rootScope.db, "CREATE TABLE IF NOT EXISTS UserMessages (id integer primary key autoincrement, userId text, name text, avatar text, messageText text, dateTimeMessage text, messageType text)").then( console.log('UserMessages table created Successfully'));
...
.state('tab', {
 ...
 templateUrl: "templates/Tabs.html"
  ...
  .state('ChannelName', {
    url: '/ChannelName',
    templateUrl: 'templates/ChannelName.html',
...
.controller('UserMessagesCtrl', ['$scope', '$rootScope', '$state',
  ...
  '$ionicPopup', '$ionicScrollDelegate', '$timeout', '$interval', '$cordovaSQLite', '$rootScope','ServiceId',
  function ($scope, $rootScope, $state, $stateParams,MockService, $ionicActionSheet, $ionicPopup, $ionicScrollDelegate, $timeout, $interval,$cordovaSQLite, $rootScope,ServiceId) {
    //angular.extend(this, $controller('ContactCnt', {$scope: $scope}));
    // mock acquiring data via $stateParams
    $scope.data =ServiceId.getData();
    console.log($scope.data.avatar+'Name from service')
   $scope.toUser={
      _id: $scope.data.userId,
      pic: $scope.data.pic,
...
$scope.messages = [{}];
    ...
    function getMessages(ID) {
     ...
     // toUserId: $scope.toUser._id
      ...
      $cordovaSQLite.execute($rootScope.db, 'SELECT name,messageText,messageType,dateTimeMessage FROM UserMessages Where userId=? ', [ID])
            .then(
                function(result) {
               
                    for(var i=0;i<result.rows.length ;i++) {
                
                console.log(result.rows.item(i).name+'Naammmee'+result.rows.item(i).userId)
                  $scope.messages[i]  = {text:result.rows.item(i).messageText,username:result.rows.item(i).name,userId:result.rows.item(i).userId,messageType:result.rows.item(i).messageType,date:result.rows.item(i).dateTimeMessage};
    ...
    message.pic = $scope.toUser.pic;
    $scope.messages.push(message);
   message.messageType='sent';
   //$cordovaSQLite.execute($rootScope.db,'delete from  UserMessages');
    $cordovaSQLite.execute($rootScope.db, 'INSERT INTO UserMessages (userId,avatar,name,messageText,dateTimeMessage,messageType) VALUES (?,?,?,?,?,?)', [message.toId,message.pic,message.tousername,message.text,message.date,'sent']);
    console.log(message.toId+'toId')
    console.log( message.tousername+'toUserName')
      ...
      message1.pic=$scope.user.pic
      message1.username=$scope.toUser.username
     message1.messageType="recieved"
     $scope.messages.push(message1);
      console.log()
    $cordovaSQLite.execute($rootScope.db, 'INSERT INTO UserMessages (userId,messageText,messageType,dateTimeMessage,name,avatar) VALUES (?,?,?,?,?,?)', [message1.userId,message1.text,'recieved',message1.date,message1.username,message1.pic]);

      
       // $scope.messages.push(MockService.getMockMessage());
     //  $cordovaSQLite.execute($rootScope.db, 'INSERT INTO UserMessages (userId,avatar,name) VALUES (?,?,?)', [$scope.toUser._id,$scope.toUser.pic,$scope.toUser.username]);
...
.controller('RecentCtrl',['$scope','$http','ServiceId1','$state',function($scope,$http,ServiceId1,$state){
  $scope.myData={}
  $scope.myObjectName=[]
  $http.get("templates/data.json").then(function (response) {
      $scope.myData = response.data;
     for( i in $scope.myData){
     $scope.myObjectName.push(i);
     console.log(i+""+$scope.myData[i])

     }
     $scope.getChannels=function(data){
        
     ServiceId1.sendData(data); 
      $state.go("ChannelName")
      }
...
.controller('ChannelNameCtrl',['$scope','$http','ServiceId1','$http','$state',function($scope,$http,ServiceId1,$http,$state){

  $scope.myChannelValues=[]

  $scope.$on('$ionicView.beforeEnter', function() {

      $scope.channelName =ServiceId1.getData()
...
$scope.myChannelValues=$scope.myData1["channel 2"]
    for(var i=0;i<$scope.myChannelValues.length;i++){
    console.log($scope.myChannelValues[i].name)
    console.log($scope.myChannelValues[i].avatar)
    console.log($scope.myChannelValues[i].userId)    
...
.controller('ContactCtrl', ['$scope', '$rootScope', '$state',
  '$stateParams', 'MockService', '$ionicActionSheet',
  '$ionicPopup', '$ionicScrollDelegate', '$timeout', '$interval', '$cordovaSQLite', '$rootScope','ServiceId',
  function ($scope, $rootScope, $state, $stateParams,MockService, $ionicActionSheet, $ionicPopup, $ionicScrollDelegate, $timeout, $interval,$cordovaSQLite, $rootScope,ServiceId) {
 $scope.$on('$ionicView.enter', function() {
...
$scope.messages1 = [{}];
 ...
 //$cordovaSQLite.execute($rootScope.db, 'delete from UserMessages');
  ...
  //$cordovaSQLite.execute($rootScope.db, 'INSERT INTO UserMessages (userId,avatar,name,messageText,dateTimeMessage,messageType) VALUES (?,?,?,?,?,?)', ['566667777744444CB','/img/image2.jpg','Ramesh','Hai Ramesh','2 seconds','sent']);

 $cordovaSQLite.execute($rootScope.db, 'SELECT Distinct userId ,name,avatar FROM UserMessages Order by dateTimeMessage').then(
               function(result) {
                for(var i=0;i<result.rows.length ;i++) {
                                    
                $scope.messages1[i]  = {username:result.rows.item(i).name,userId:result.rows.item(i).userId,pic:result.rows.item(i).avatar};
                       console.log($scope.messages1[i].username+""+$scope.messages1[i].userId+""+$scope.messages1[i].pic) 
                       console.log($scope.messages1.length)
                }
                }).then( function() {
                   var len = $scope.messages1.length;      
                   for(var i=0;i<len ;i++) {

                   (function(i){
                    // console.log('value of  userId'+$scope.messages1[i].userId)
                   $cordovaSQLite.execute($rootScope.db, 'SELECT dateTimeMessage, messageText FROM UserMessages where userId=?',[$scope.messages1[i].userId]).then(
                   function(result1) {
                     console.log(result1.rows.length-1+'Message Text')
                     $scope.messages1[i].messageText=result1.rows.item(result1.rows.length-1).messageText;
                     $scope.messages1[i].dateTimeMessage=result1.rows.item(result1.rows.length-1).dateTimeMessage;
                   })
                 })(i)
            }    
          });
                    
   return $scope.messages1;      
  ...
  var id=''
 $scope.userID=function (message){
   alert('Hai')
  id= message.userId;
  ServiceId.sendData(message)
 }
}])

// services
.factory('MockService', ['$http', '$q','$cordovaSQLite', '$rootScope',
  function($http, $q,$cordovaSQLite,$rootScope) {
    var me = {};
